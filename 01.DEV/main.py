...
class VideoPlayer:
    ...
    def calculate_interaction_progress(self):
        if MODE == "button":
            self.interaction_progress = min(self.button_counter / int(BUTTON_PRESS_THRESHOLD), 1.0)
        elif MODE == "encoder":
            self.interaction_progress = min(self.encoder_counter / int(ENCODER_THRESHOLD), 1.0)
        else:
    ...
    def on_button_press(self):
        ...
        if self.state in ["loop", "action"]:
            ...
            print(f"[BUTTON] Clic détecté : {self.button_counter}/{int(BUTTON_PRESS_THRESHOLD)}")
    ...
    def on_encoder_rotate(self):
        if self.state in ["loop", "action"]:
            ...
            self.last_rotation_time = time.time()
            print(f"[ENCODER] Rotation détectée : {self.encoder_counter}/{int(ENCODER_THRESHOLD)}")
